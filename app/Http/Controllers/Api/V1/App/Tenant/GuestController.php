...
class GuestController extends ApiController
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        if (!Auth::attempt(['email' => $attributes['email'], 'password' => $attributes['password'], 'role' => User::RESTAURANT_ADMIN_ROLE])) {
            return $this->errorResponse('Invalid login details', Response::HTTP_UNAUTHORIZED);
        }
        $user = $this->userRepository->where('email', $attributes['email'])->where('role', User::RESTAURANT_ADMIN_ROLE)->first();
