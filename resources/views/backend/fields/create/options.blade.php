<div class="form-group">
    ...
    @for($i = 0; $i <= 3; $i++)
        <input type="text" name="options[{{ $i }}][name]"
               class="form-control {{ $errors->has('options'. $i . '.name') ? 'is-invalid' : '' }}"
        ...
        <div class="form-check">
            <input type="radio" name="options[{{ $i }}][is_answer]" value="1"
                   {{ old('options.'. $i . '.is_answer') == 1 ? 'checked' : '' }} class="form-check-input"
                   id="options.{{ $i }}.is_answer">
